...
function normalizeRow(r) {
  return {
    ...
    patientName: r.patientName ?? r.userName ?? r.user_name ?? r.patientNm ?? "-",
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "right", marginTop: "2rem" }}>
          <button className="hdp-btn hdp-btn-primary" style={{ width: "100%" }} onClick={onClose}>
            닫기
          </button>
        </div>
      </div>
  ...
  const fetchReservations = async () => {
    try {
      ...
      if (e?.response?.status === 401) {
        ...
        setLoadError(e?.response?.data?.message || e?.message || "예약 목록을 불러오지 못했습니다.");
      }
      setDataList([]);
      setTotal(0);
      setTotalPages(1);
  ...
  const filtered = useMemo(() => {
    ...
    return dataList.filter((row) => {
      ...
      return (
        ...
        String(row.hospitalName ?? "").toLowerCase().includes(kw)
      );
    });
    ...
    <div className="adm-page">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "2.5rem" }}>
        <div>
          <div style={{ color: "var(--text-muted)", fontSize: "0.85rem", fontWeight: 600, marginBottom: "0.5rem" }}>
            <i className="fas fa-home" style={{ marginRight: "4px" }} /> 예약관리{" "}
      ...
      <div className="adm-card" style={{ padding: "1.5rem 2rem" }}>
        <div style={{ display: "flex", gap: "2rem", alignItems: "center" }}>
              ...
              {STATUS_OPTIONS.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    background:
                      statusFilter === s ? "linear-gradient(135deg, var(--primary-mint), var(--primary-teal))" : "#fff",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", padding: "0 1rem", color: "var(--primary-teal)" }}>
      ...
      <div className="adm-table-wrap" style={{ overflowX: "auto" }}>
        ...
        >
          <span style={{ fontSize: "0.9rem", fontWeight: 700, color: "var(--text-secondary)" }}>
            검색 내역 <span style={{ color: "var(--primary-teal)" }}>{total}</span>건
        ...
        <table className="adm-table" style={{ tableLayout: "fixed", width: "100%", minWidth: "1400px" }}>
          ...
          <tbody>
            {!loading && filtered.length === 0 ? (
              <tr>
                <td colSpan={10} style={{ textAlign: "center", padding: "2.5rem 1rem", color: "var(--text-muted)", fontWeight: 700 }}>
                  표시할 예약이 없습니다.
                </td>
              </tr>
            ) : (
              filtered.map((r, idx) => (
                <tr key={r.id ?? idx} style={{ height: "70px" }}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <span style={{ padding: "0.2rem 0.5rem", background: "var(--bg-tertiary)", color: "var(--primary-dark-teal)", borderRadius: "4px", fontSize: "0.75rem", fontWeight: 700 }}>
                      {r.department}
                    </span>
                  </td>
                  ...
                  <td style={{ textAlign: "left" }}>
                    <div
                      ...
                      title="클릭해서 전체 보기"
                    >
                      <i className="far fa-comment-dots" style={{ marginRight: "6px" }} /> {r.requestMemo || "요청사항 없음"}
                    </div>
                  </td>
                  ...
                  <td style={{ textAlign: "center", padding: 0 }}>
                    <div style={{ display: "flex", gap: "6px", justifyContent: "center", alignItems: "center", height: "100%" }}>
                      {r.status === "예약대기" ? (
                        <>
                          <button
                            onClick={() => handleStatusChange(r.id, "예약확정")}
                            style={{
                              width: "55px",
                              height: "32px",
                              background: "#dcfce7",
                          ...
                          <button
                            onClick={() => handleStatusChange(r.id, "예약취소")}
                            style={{
                              width: "55px",
                              height: "32px",
                              background: "#fee2e2",
        ...
        {/* 페이지네이션 */}
        <div style={{ padding: "1.5rem", display: "flex", justifyContent: "center", alignItems: "center", gap: "1.25rem", background: "#fff", borderTop: "1px solid var(--border-color)" }}>
          <button
            disabled={page === 1}
