...
export function WebSocketProvider({ children }) {
  ...
  const getRoomsUrl   = (u) => isAdmin(u)
    ? `${API}/api/chat/rooms/hospital/${u.hoNum}`
    : `${API}/api/chat/rooms/${u.num}`;
  const getAlarmChannel = (u) => isAdmin(u)
    ? `/topic/alarm.hospital_${u.hoNum}`
    : `/topic/alarm.${u.num}`;
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        setConnected(true);
        console.log("✅ STOMP 연결됨 - role:", user.role, "| hoNum:", user.hoNum);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/chat/rooms/${roomId}/read?reader=${reader}`, { method: "PUT" })
