...
const buildWeeklyLines = (hoursArr) => {
  ...
  const map = new Map(hoursArr.map((h) => [h.hhDayOfWeek ?? h.dayOfWeek ?? h.day ?? h.hh_day_of_week, h]));
    ...
    <span className="hd2__stars">
      {Array.from({ length: 5 }, (_, i) => {
        ...
        return (
          <i
            key={i}
            className={fill >= 1 ? "fas fa-star" : fill >= 0.5 ? "fas fa-star-half-stroke" : "far fa-star"}
          />
        );
      })}
    </span>
...
export default function HospitalDetail() {
  ...
  useEffect(() => {
    ...
    async function loadReviews() {
      ...
      try {
        const revRes = await fetch(`${API_BASE_URL}/api/v1/hospitals/${hospitalId}/reviews?limit=${reviewLimit}`);
  ...
  if (loading) return <div className="hd2__fullstate"><i className="fas fa-spinner fa-spin" /><span>병원 정보를 불러오는 중...</span></div>;
  if (error) return <div className="hd2__fullstate hd2__fullstate--err"><p>{error}</p><button onClick={() => navigate(-1)}>목록으로 돌아가기</button></div>;

  return (
    <div className="hd2">
      {/* ══ 히어로 섹션 ══ */}
      <div className="hd2__hero">
        <div className="hd2__hero-blob" /><div className="hd2__hero-blob hd2__hero-blob--2" />
        <div className="hd2__hero-inner">
          ...
          <div className="hd2__hero-content">
            ...
            <div className="hd2__hero-text">
              {summary?.deptName && <span className="hd2__hero-dept"><i className="fas fa-stethoscope" /> {summary.deptName}</span>}
              ...
              <div className="hd2__hero-rating">
                ...
                <span className="hd2__hero-rcnt">{totalReviewCount}개 리뷰</span>
              </div>
      ...
      {summary && (
        <div className="hd2__strip">
          <div className="hd2__strip-inner">
            <span className="hd2__strip-item"><i className="fas fa-location-dot" /> {summary.hoAddr}</span>
            <span className="hd2__strip-sep" /><span className="hd2__strip-item"><i className="far fa-clock" /> {timeText}</span>
            {lunchText && <><span className="hd2__strip-sep" /><span className="hd2__strip-item"><i className="fas fa-utensils" /> {lunchText}</span></>}
            <span className="hd2__strip-sep" /><span className="hd2__strip-item"><i className="fas fa-phone" /><a href={`tel:${summary.hoPhone}`} className="hd2__strip-tel">{summary.hoPhone}</a></span>
          </div>
        </div>
      )}
      ...
      <div className="hd2__wrap">
        <aside className="hd2__left">
          ...
          <section className="hd2__leftCard">
            <div className="hd2__cardTitle"><i className="fas fa-circle-info" /> 병원 정보</div>
            {summary && (
              <>
                ...
                <ul className="hd2__infoList">
                  <li className="hd2__infoLine"><i className="fas fa-phone hd2__infoIcon" />{summary.hoPhone}</li>
                  <li className="hd2__infoLine"><i className="fas fa-location-dot hd2__infoIcon" /><span>{summary.hoAddr}</span></li>
                  {weeklyLines.length > 0 && (
                    <li className="hd2__infoLine hd2__infoLine--week">
                      ...
                      <div className="hd2__week">
                        {weeklyLines.map((line, idx) => <div key={idx} className="hd2__weekLine">{line}</div>)}
                      </div>
                    </li>
                  )}
                </ul>
          ...
          <div className="hd2__reserveCard">
            <div className="hd2__reserveHead"><i className="fas fa-calendar-check" /> 빠른 예약</div>
            <p className="hd2__reserveDesc">지금 바로 온라인으로<br />간편하게 예약하세요</p>
            <button className="hd2__actionBtn hd2__actionBtn--primary" onClick={handleReserveOpen}>
              <i className="fas fa-calendar-plus" /> 예약하기
            </button>
            <button className="hd2__actionBtn hd2__actionBtn--ghost" onClick={handleInquiry}>
        ...
        <main className="hd2__right">
          ...
          <section className="hd2__panel">
            <div className="hd2__panelTitle"><span className="hd2__panelIcon"><i className="fas fa-camera" /></span> 병원 사진</div>
          ...
          {/* 지도 패널 */}
          <section className="hd2__panel">
            <div className="hd2__panelTitle"><span className="hd2__panelIcon"><i className="fas fa-map-location-dot" /></span> 오시는 길</div>
            <div className="hd2__addrBar"><i className="fas fa-location-dot" /> {summary?.hoAddr ?? "주소 정보 없음"}</div>
            <div className="hd2__panelBody hd2__mapBox">
              <KakaoMap
                center={{ lat: summary?.hoLat ?? summary?.ho_lat, lng: summary?.hoLng ?? summary?.ho_lng }}
                markers={[{ id: hospitalId, lat: summary?.hoLat ?? summary?.ho_lat, lng: summary?.hoLng ?? summary?.ho_lng }]}
                level={3} height={280}
              />
            </div>
          </section>
          ...
          <section className="hd2__panel">
            <div className="hd2__panelTitle">
              <span className="hd2__panelIcon"><i className="fas fa-star" /></span> 리뷰
              {totalReviewCount > 0 && <span className="hd2__reviewBadge">{totalReviewCount}</span>}
            </div>
            <div className="hd2__reviews">
              {reviews.length === 0 ? (
                <div className="hd2__emptyReviews"><i className="far fa-comment-dots" /><p>등록된 리뷰가 없습니다</p></div>
              ) : (
                reviews.map((rev) => (
                  <div key={rev.rvNum} className="hd2__reviewRow">
                    <div className="hd2__revAvatar">
                      {rev.rfName ? <img src={`${API_BASE_URL}/uploads/${rev.rfName}`} alt="리뷰" className="hd2__revAvatarImg" /> : <i className="fas fa-user" />}
                    </div>
                    <div className="hd2__reviewContent">
                      <div className="hd2__revTop">
                        <span className="hd2__revUser">{rev.userName}님</span>
                        <div className="hd2__revRating"><Stars score={rev.rvRating ?? 0} /><span className="hd2__revScore">{Number(rev.rvRating ?? 0).toFixed(1)}</span></div>
                      </div>
                      <p className="hd2__reviewText">{rev.rvContent}</p>
                      <span className="hd2__revDate"><i className="far fa-calendar" /> {formatDate(rev.rvCreatedAt)}</span>
                    </div>
                  </div>
                ))
              )}
            ...
            {totalReviewCount > STEP && reviews.length < totalReviewCount && (
              <div className="hd2__moreWrap">
                <button className="hd2__moreBtn" type="button" disabled={reviewLoading} onClick={() => setReviewLimit((prev) => prev + STEP)}>
